...
The v2 retrain of the CNN+BiLSTM nowcaster (Ledger #1) initially suffered from a high false alarm rate (FAR ~59%). This document proposed a multi-layer mitigation architecture to address this. **As of Phase F, Layers 1 and 2 (Focal Loss + Feature Enrichment) were massively successful.** By expanding the model to 18 features (including 7-day soil saturation, spatial contrast, and explicit satellite `uth_valid` masking) and applying a strict 7-day Purge Boundary split, the single-stage CNN+BiLSTM achieved a stunning **5.98% FAR (94% precision)** on the clean test split. Consequently, Layer 3 (Two-Stage Cascade) has been parked as the FAR target was vastly exceeded natively.
...
|:---|:---:|:---:|:---:|
| Misses (False Negatives) | 262 | 121 | −141 (−53.8%) |
| False Alarms (False Positives) | 2,638 | 3,278 | +640 (+24.3%) |
| POD | 88.75% | 94.80% | +6.05 pts |
| FAR | 56.15% | 59.75% | +3.60 pts |
| CSI | 0.4154 | 0.3938 | **−0.0216 (net regression)** |
...
### Layer 3 — Two-Stage Precision Cascade (PARKED)

> [!TIP]
> **FAR Target Achieved Natively:** As of the Phase F evaluation, the single-stage CNN+BiLSTM (using 18 augmented features, Focal Loss, and the 7-day Purge Boundary) natively achieved a **5.98% FAR** (CSI: 0.8832). Because the model already achieves ~94% precision directly from the primary pass, this complex secondary cascade is officially **parked** to minimize inference latency and architectural debt. The original design is preserved below for institutional memory.
...
**Station-density audit (required before Level 2 finalisation):** Document 09's verified basin positive counts (Rudraprayag: 114, Chamoli: 107, Tehri: 154 — versus Assam: 7,866) confirm coverage is highly uneven. Some high-risk Uttarakhand catchments may lack two independent stations near the actual risk zone. Before finalising the dual-station consensus rule, audit the AWS network station count and inter-station distances per basin. Where a basin cannot structurally satisfy the dual-station requirement, define an explicit fallback so Level 2 does not silently become unreachable in exactly the basins most likely to need it.
...
| v3 | 2026-09-15 | **Execution Update:** Documented the success of Layer 1 (Focal Loss) and Layer 2 (18-Feature Enrichment + 7-Day Purge Split). The CNN+BiLSTM achieved a native 5.98% FAR (CSI 0.8832), vastly exceeding expectations and rendering the Layer 3 Two-Stage Cascade obsolete. Layer 3 officially parked. |
